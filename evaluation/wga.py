...
# Compute I-AUROC at different levels of granularity.
...
def _auroc_by_group(df: pd.DataFrame,
    ...
    """
    Helper: compute mean I-AUROC grouped by a single column.
...
def wga_by_category(df_dict: dict) -> pd.DataFrame:
    """
    Level 1: I-AUROC per object category for each model.
...
def wga_by_viewpoint(df_dict: dict) -> pd.DataFrame:
    """
    Level 2: I-AUROC per camera viewpoint for each model.
...
def wga_by_defect_type(df_dict: dict) -> pd.DataFrame:
    """
    Level 3: I-AUROC per defect type for each model.
...
# Note: this will be moved to visualisation.py in the refinement pass
# =============================================================================

def plot_wga_heatmap(df_dict: dict,
                     output_path: Optional[str] = None) -> None:
    """
    Plot a heatmap comparing I-AUROC across categories and viewpoints
    for all three models side by side.
    Low AUROC = red (struggling), high AUROC = green (performing well).

    Args:
        df_dict:     {'ModelName': results_df, ...}
        output_path: path to save figure, None = display inline
    """
    ...
    fig.colorbar(im, ax=axes, label='I-AUROC')
    fig.suptitle('Worst-Group Analysis: I-AUROC by Category × Viewpoint',
